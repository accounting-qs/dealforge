...
2. **Adjacent-industry taxonomy fetch (grounded).** Before the planner, call `suggestIndustries(seedIndustry)` (server.js:3284 — free, no Apollo credits) to get canonical Apollo industry names co-occurring with the seed. `adjacentIndustries` = `[seed, ...live]`, deduped, capped at 6 (e.g. `pharmaceuticals` → `pharmaceuticals, biotechnology, hospital & health care, research, medical devices, health/wellness`). This is the "LLM picks from real Apollo options" path — the values are guaranteed valid.
